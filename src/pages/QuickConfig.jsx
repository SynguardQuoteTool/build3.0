  ...
  const availableControllerData = useMemo(() => {
        ...
        if (deployment === 'In') {
            ...
            if (commsType === 'RS-485') {
                ...
                caps = {
                    ...
                    'SynConSC': { doors: 1, doorsInOnly: 2, readers: 2, wiegand: 2, requiresSynApp: true } // Added doorsInOnly
                };
            }
        }
    } else if (protocol === 'OSDP') {
    ...
    return mdls.map(m => {
        ...
        return {
            ...
            effectiveDoorsInOnly: e.doorsInOnly ?? e.doors ?? b?.baseDoors ?? 0, // Store "In" capacity for Wiegand In & out
...
useEffect(() => {
  ...
  // For Wiegand In & out, include the "In" capacity in the display
  const capacitiesWithLabels = uniqueCapacities.map(capacity => {
      const controller = controllers.find(c => c.effectiveDoors === capacity);
      if (protocol === 'Wiegand' && deployment === 'In & out' && controller.effectiveDoorsInOnly && controller.effectiveDoorsInOnly !== capacity) {
          return {
              value: capacity,
...
}, [availableControllerData, targetDoorsPerController, protocol, deployment]);
          ...
          <div className="config-box">
            ...
            <label>
              ...
              <select
                 ...
                 onChange={e => setTargetDoorsPerController(e.target.value)}
                 disabled={!commsType || availableTargetCapacities.length === 0}
              >
